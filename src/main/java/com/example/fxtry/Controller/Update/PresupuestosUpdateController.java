...
import static com.example.fxtry.Controller.PresupuestosController.presupuestoToUpdate;

public class PresupuestosUpdateController {
    ...
    private void initialize() {
        ...
        // Load existing data if presupuestoToUpdate is set
        if (presupuestoToUpdate != null) {
            // Set the location
            txtUbicacion.setText(presupuestoToUpdate.getUbicacion());

            // Set the rejected status
            chkRechazado.setSelected(presupuestoToUpdate.getRechazado());

            // Select the garden in the combo box
            if (presupuestoToUpdate.getIdJardin() != null) {
                try {
                    JardinesDTO jardin = implRetroFit.getJardine(presupuestoToUpdate.getIdJardin());
                    if (jardin != null) {
                        for (JardinesDTO j : cmbJardin.getItems()) {
                            if (j.getId().equals(jardin.getId())) {
                                cmbJardin.setValue(j);
                                break;
                            }
                        }
                    }
                } catch (IOException e) {
                    lblError.setText("Error al cargar jardín: " + e.getMessage());
                    e.printStackTrace();
                }
            }

            // Select the solicitud in the combo box
            if (presupuestoToUpdate.getIdSolicitud() != null) {
                try {
                    SolicitudDTO solicitud = implRetroFit.getSolicitud(presupuestoToUpdate.getIdSolicitud());
                    if (solicitud != null) {
                        for (SolicitudDTO s : cmbSolicitud.getItems()) {
                            if (s.getId().equals(solicitud.getId())) {
                                cmbSolicitud.setValue(s);
                                break;
                            }
                        }
                    }
                } catch (IOException e) {
                    lblError.setText("Error al cargar solicitud: " + e.getMessage());
                    e.printStackTrace();
                }
            }
        }
    ...
    private void update(ActionEvent event) {
        ...
        try {
            if (presupuestoToUpdate == null) {
                lblError.setText("No hay presupuesto para actualizar.");
                return;
            }
            ...
            if (selectedJardin != null) {
                presupuestoToUpdate.setIdJardin(selectedJardin.getId());
            }

            // Set the solicitud ID
            SolicitudDTO selectedSolicitud = cmbSolicitud.getValue();
            if (selectedSolicitud != null) {
                presupuestoToUpdate.setIdSolicitud(selectedSolicitud.getId());
            }

            // Set the location
            presupuestoToUpdate.setUbicacion(txtUbicacion.getText().trim());

            // Set the rejected status
            presupuestoToUpdate.setRechazado(chkRechazado.isSelected());

            // Update the presupuesto
            implRetroFit.updatePresupuesto(presupuestoToUpdate.getId(), presupuestoToUpdate);

            AlertController.showInformation("Presupuesto Actualizado", "El presupuesto ha sido actualizado correctamente.");
            navigateToPresupuestosView(event);
        } catch (IOException e) {
            lblError.setText("Error al actualizar el presupuesto: " + e.getMessage());
            e.printStackTrace();
        }
    }
