...
public class PeticionesCreateController {
    ...
    private void create(ActionEvent event) {
        ...
        try {
            ...
            solicitudDTO.setFechaSolicitud(ZonedDateTime.now());
            solicitudDTO.setIdUsuario(Integer.parseInt(txtIdUsuario.getText().trim()));
            solicitudDTO.setDescripcion(txtDescripcion.getText().trim());
            solicitudDTO.setAtendida(false);

            implRetroFit.createSolicitud(solicitudDTO);

            AlertController.showInformation("Solicitud Creada", "La solicitud ha sido creada correctamente.");
            navigateToPeticionesView(event);
        } catch (NumberFormatException e) {
            lblError.setText("Error: El ID de usuario debe ser un número entero.");
            e.printStackTrace();
        } catch (IOException e) {
            lblError.setText("Error al crear la solicitud: " + e.getMessage());
            e.printStackTrace();
        }
    }
